...
class ActionNode(Node):
    ...
    def sample_state(self, real_world=False):
        ...
        be set to True if a real world action is taken. The belief is then
        used from the real world action instead from the belief state actions.
        :return: The state node, which was sampled.
        """
        """
        if real_world:
            state = self.parent.real_world_perform(self.action)
        else:
        ...
        """
        # deterministic, no need for sampling!
        return self.parent.perform(self.action)
...
class StateNode(Node):
    ...
    def is_terminal(self):
        return not (np.any(self.state[:,:,0]) or \
                    np.any(self.state[:,:,2]) or \
                    np.any(self.state[:,:,4]))
